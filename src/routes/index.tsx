...
// Hand-authored loss curve: steep early drop, a couple of realistic noisy
// bumps, and a converging tail plateau. Purely decorative — no training here.
const LOSS_CURVE = [
  4.82, 4.51, 4.3, 4.05, 3.72, 3.51, 3.3, 3.1, 2.95, 2.8, 2.6, 2.45, 2.3, 2.1,
  1.95, 1.85, 1.92, 1.78, 1.6, 1.52, 1.45, 1.38, 1.3, 1.22, 1.15, 1.1, 1.02,
  0.98, 0.92, 0.88, 0.94, 0.83, 0.79, 0.75, 0.71, 0.68, 0.64, 0.61, 0.58,
  0.55, 0.52, 0.5, 0.47, 0.45, 0.43, 0.41, 0.39, 0.37, 0.35, 0.33, 0.31, 0.3,
  0.29, 0.28, 0.27, 0.255, 0.24, 0.23, 0.22, 0.21, 0.205, 0.2, 0.19, 0.185,
  0.18, 0.175, 0.17, 0.165, 0.16, 0.155, 0.15, 0.145, 0.14, 0.135, 0.13,
  0.125, 0.12, 0.115, 0.11, 0.105, 0.1, 0.098, 0.095, 0.093, 0.09, 0.088,
  0.085, 0.083, 0.08, 0.078, 0.076, 0.074, 0.072, 0.07, 0.068, 0.065, 0.062,
  0.06, 0.058, 0.055, 0.052, 0.05,
];
...
function GradientDescentScrollbar() {
  const progress = useScrollProgress();
  const n = LOSS_CURVE.length;
  const trackHeight = 260;
  const trackWidth = 46;
  const baseX = 30;
  const amplitude = 18;
  const maxLoss = LOSS_CURVE[0];

  const points = LOSS_CURVE.map((loss, i) => ({
    x: baseX - (loss / maxLoss) * amplitude,
    y: (i / (n - 1)) * trackHeight,
  }));
  ...
  const currentLoss = LOSS_CURVE[activeIndex];
  ...
  return (
    <div className="fixed right-2 top-1/2 z-40 hidden -translate-y-1/2 lg:block" aria-hidden="true">
      <svg
        width={trackWidth}
        height={trackHeight}
      ...
      >
        <path d={toPath(points)} fill="none" stroke="var(--ink)" strokeOpacity="0.18" strokeWidth="1.2" />
        <path d={toPath(points.slice(0, activeIndex + 1))} fill="none" stroke="var(--marker-strong)" strokeWidth="1.6" />
        <circle cx={activePoint.x} cy={activePoint.y} r="3" fill="var(--ink)" />
      </svg>
      <div
        className="pointer-events-none absolute whitespace-nowrap border border-ink bg-paper px-1.5 py-0.5 text-[0.62rem]"
        style={{
          top: `${activePoint.y}px`,
          right: `${trackWidth + 6}px`,
          transform: "translateY(-50%)",
          fontFamily: "var(--font-mono-paper)",
        }}
      >
        loss: {currentLoss.toFixed(2)}
        {progress > 0.97 ? " (converged)" : ""}
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    const compute = () => {
      const els = document.querySelectorAll<HTMLElement>(
        "h1, h2, .marker, figcaption .font-bold, .paper-tab",
      );
...
function AttentionLines() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      frame.current = requestAnimationFrame(() => setCursor({ x: e.clientX, y: e.clientY }));
    };
  ...
  return (
    <svg className="pointer-events-none fixed inset-0 z-30 hidden h-full w-full lg:block" aria-hidden="true">
      {nearby.map((t, i) => {
        const opacity = 0.28 * (1 - t.dist / RADIUS);
